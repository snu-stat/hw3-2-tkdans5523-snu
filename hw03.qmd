...
- 또한 모든 변수의 95% 신뢰구간이 1을 포함하지 않으므로, 선택된 6개의 변수 모두 20세 이상 성인의 현재 흡연 여부를 예측하는 데 통계적으로 유의한 변수로 볼 수 있다.
...
- 제시된 공식에 사용되는 변수들을 기존 데이터셋에 추가한다.(`WPct`, `RS`, `RA`)
...
- 로지스틱 회귀 모형의 일반적인 형태는 $\text{logit}(Y) = \beta_0 + \beta_1 X$이다. 
- 위 공식과 형태를 비교해 보면 다음과 같은 모형을 세울 수 있다. $\text{logit}(Y) = \beta_1 X$
- 이렇게 모형을 세우고 적합하면, 구하고자 하는 회귀계수 $\beta_1$이 구조적으로 Bill James 공식의 모수 $k$와 거의의 같은 의미를 가지게 된다. 
    - 반응변수: 승률 (`WPct`)
    - 설명변수: 득실점 비율의 로그값 (`log(run_ratio)`)
    - 절편 ($\beta_0$): $0$ (절편이 없는 모형)
- R의 glm() 함수에서 반응변수로 0과 1로 이루어진 이진 데이터가 아닌 '비율 데이터(승률)'를 사용할 때는 총 경기 수(W + L)를 가중치(weights)로 지정해 주어야 한다. 
- 로지스틱 회귀는 내부적으로 이항 분포를 가정하기 때문에, 단순한 승률 수치(예: 0.5)만으로는 데이터의 신뢰도를 알 수 없다. 예를 들어, 0.5의 승률이 10경기에서 나온 것인지 100경기에서 나온 것인지에 따라 그 승률의 신뢰도가 달라지므로, 총 경기 수를 가중치로 지정하여 모델이 각 관측치의 신뢰도를 반영할 수 있도록 해야 한다.
...
- 추정치는 `nls` 모형과 `glm` 모형 모두 소수점 둘째 자리까지 동일함을 알 수 있고, 신뢰구간도 거의 겹치는 것을 볼 수 있다.
- 표준오차(Std_Error)의 경우, `nls` 모형은 오차의 분산이 항상 일정하다고 가정하는 반면, `glm` 모형은 승률 같은 '비율 데이터'가 0.5에 가까울수록 분산이 커진다는 이항 분포의 특성을 반영하기 때문에, `glm` 모형의 표준오차가 조금 더 크고 신뢰구간이 더 넓게 계산된 것으로 보인다.
...
ggsave("figure/R_residual_plot.png", plot_resid, width = 12, height = 8, dpi = 150)
ggsave("figure/R_predicted_plot.png", plot_pred, width = 12, height = 8, dpi = 150)
```

![](figure/R_residual_plot.png)
![](figure/R_predicted_plot.png)
...
# 두 그래프를 나란히 출력 (plotnine 0.15+ 내장 composition)
plot_resid.save("figure/P_residual_plot.png", width=12, height=8, dpi=150)
plot_pred.save("figure/P_predicted_plot.png", width=12, height=8, dpi=150)
```

![](figure/P_residual_plot.png)
![](figure/P_predicted_plot.png)
...
- Deviance residuals vs linear predictors ($\eta$) 산점도 
    ...
    - 결과 : 점들이 없이 무작위하게 분포하고 있다. 특정한 곡선 패턴이나 이분산성(분산이 달라지는 현상)이 관찰되지 않으므로, 잔차의 관점에서 로지스틱 회귀 모형이 데이터에 잘 적합되었다라고 판단할 수 있다.
- 관측된 WPct와 모형에서 예측하는 WPct를 산점도 그래프로 비교
    ...
    - 결과 : 점들이 $y=x$대각선을 따라 매우 조밀하게 밀집되어 있다. 특정 구간에서 크게 벗어나는 편향 없이, 모형이 추정한 승률이 실제 승률을 잘 예측하고 있다고 할 수 있다.

- 결론: 두 그래프 결과와 Residual Deviance 검정 결과를 종합해 보면, 로지스틱 회귀 모형이 잘 적합되었다고 판단할 수 있다.
...
- 4번에서 요구되는 모형은 다음과 같다.
$$\operatorname{logit}(W P c t)=\beta_{R A} \log (R A)+\beta_{R S} \log (R S)$$

- 앞선 두 모형과 비교해 볼 때, 4번 모형이 두 모형과 유사하다면 다음 관계가 성립할 수 있다.
$$
\beta_{R S} \approx \beta_1( \ \ \text{or} \  k), \quad \beta_{R A} \approx -\beta_1( \ \ \text{or} \  -k)
$$
...
```{python}
# 변수 추가 및 모형 적합
teams_model4 = teams_filtered.assign(
    log_RA=np.log(teams_filtered["RA"]),
    log_RS=np.log(teams_filtered["RS"])
)
...
- 이제 1번의 $k$, 2번의 $\beta_1$, 그리고 4번 모형에서 얻은 추정 결과를 모두 출력하여 비교해보자.
...
- 먼저 문제 2-2에 명시된 `문제1`을 `문제2-1`로 생각하겠다.
- 앞서, `문제2-1`에서 3가지 모형을 적합하였다.
- 분석 결과 3가지 모형 모두 같은 의미를 가지는 모형이라고 판단하였다.
- 따라서, `문제2-1`의 3가지 모형 중 세번째 모형(`model_logit_rs_ra`)만 가지고, 문제 2-2에서 요구되는 모형과 비교하겠다.
...
# 강의에서는 stepAIC를 사용하였지만, 여기서는 step() 함수를 사용하였다.
# step() 함수는 AIC를 default metric으로 설정되어 있다.
...
```{python}
# python에서 AIC 계산 과정이 R과 달라 새롭게 정의의
def glm_aic_r(model, data):
    """R AIC(glm)과 동일: -2 * logLik + 2 * df"""
    mu = np.clip(model.fittedvalues, np.finfo(float).eps, 1 - np.finfo(float).eps)
    n_trials = data["W"] + data["L"]
    log_lik = binom.logpmf(data["W"], n_trials, mu).sum()
    n_params = len(model.params)
    return -2 * log_lik + 2 * n_params, n_params

# Python에서 로지스틱 회귀 모형 적합하는 함수수
...
# Python에서 AIC 계산하는 함수 (R AIC(glm)과 동일)
def _model_aic(model, data, weights=None):
    if weights is not None:
        return glm_aic_r(model, data)[0]
    return -2 * model.llf + 2 * len(model.params)

# Python에서 Stepwise 변수 선택하는 함수 (R step(..., direction="both")과 동일)
def stepwise_selection(data, target, predictors, family, weights=None):
    """매 iteration마다 add/remove 후보 중 AIC가 가장 낮은 모형 1개만 선택."""
...
trial_weights = df["W"] + df["L"]

# R의 full_model + step()에 대응
full_model = _fit_glm(df, "WPct", PREDICTORS, sm.families.Binomial(), trial_weights)
step_model, selected_vars = stepwise_selection(
    df, "WPct", PREDICTORS, sm.families.Binomial(), weights=trial_weights
)
...
output_final <- paste0(
  ...
  "Comparison with model_logit_rs_ra based on AIC\n",
  paste(capture.output(print(round(AIC(model_logit_rs_ra, final_model), 4))), collapse = "\n"), "\n"
)
...
# model_logit_rs_ra 및 final_model AIC (R과 동일한 방식)
model4_aic, model4_df = glm_aic_r(model_logit_rs_ra, teams_model4)
final_aic, final_df = glm_aic_r(final_model, df)

aic_comparison = pd.DataFrame({
    "df": [model4_df, final_df],
    "AIC": [round(model4_aic, 4), round(final_aic, 4)]
}, index=["model_logit_rs_ra", "final_model"])
...
- `문제2-1` 4번 모형(`model_logit_rs_ra`: 절편 없는 `log(RA)`, `log(RS)` 로지스틱 모형)과 AIC를 비교하면, stepwise 선택 후 `CG`를 제거한 최종 모형(`final_model`)의 AIC가 더 낮게 나타난다.
- 이는 `logRS`, `logRA` 외에 `SHO`, `SV`를 추가하면 승률 설명력이 개선되어, 단순히 득·실점 로그만으로 구성된 4번 모형보다 데이터에 더 잘 적합함을 시사한다.
...
def stepwise_selection(data, target, predictors, family, weights=None):
    """R step(..., direction='both')과 동일: iteration마다 변수 1개만 add/remove."""
    current_predictors = predictors.copy()
    best_model = _fit_glm(data, target, current_predictors, family, weights)
    while True:
        best_aic = _model_aic(best_model, data, weights)
...
# df는 문제 2-2에서 생성; 반응변수만 W로 포아송 stepwise 수행
full_poisson = _fit_glm(df, "W", PREDICTORS, sm.families.Poisson())
step_poisson, selected_vars_poisson = stepwise_selection(
    df, "W", PREDICTORS, sm.families.Poisson()
)

print(step_poisson.summary())
print("selected:", selected_vars_poisson)
```
:::

- **해석 기준 (stepwise)**: R `step()`과 Python `stepwise_selection()`의 선택 변수 집합이 동일하면(예: `logRS`, `ERA`, `CG`, `E`, `SV`) 두 언어에서 같은 stepwise 결과가 나온 것이다. 각 계수의 점추정치가 소수점 둘째~셋째 자리까지 같으면 추정도 일치한 것으로 본다.
...
final_poisson = smf.glm(
    formula="W ~ logRS + ERA + SV",
    data=df,
    family=sm.families.Poisson(),
).fit()

summary_final_poisson = pd.DataFrame({
    "Estimate": final_poisson.params,
    "Std. Error": final_poisson.bse,
    "z value": final_poisson.tvalues,
    "Pr(>|z|)": final_poisson.pvalues,
}).round(4)

logistic_aic, _ = glm_aic_r(final_model, df)
poisson_aic = -2 * final_poisson.llf + 2 * len(final_poisson.params)

aic_comparison = pd.DataFrame({
    "AIC": [round(logistic_aic, 4), round(poisson_aic, 4)]
}, index=["final_model (logistic)", "final_poisson (Poisson)"])

output_compare = (
    "Summary final_poisson\n"
    f"{summary_final_poisson.to_string()}\n\n"
    "AIC Comparison\n"
    f"{aic_comparison.to_string()}\n"
)
...
- **해석 기준 (최종 포아송 모형)**: R `summary(final_poisson)`과 Python 계수표에서 `logRS`, `ERA`, `SV`의 부호·크기·p-value가 같으면(소수점 셋째 자리까지) 동일하게 적합된 것이다. `문제 2-1` 2항처럼 모형 표기는 다르지만 계수가 같으면 같은 결론을 내린다.
- **해석 기준 (AIC 비교)**: 로지스틱 `final_model` AIC와 포아송 `final_poisson` AIC는 반응변수(승률 vs 승수)와 분포가 달라 직접 비교하지 않는다. 두 AIC 수치가 모두 출력되더라도, 더 낮다고 해서 한쪽 모형이 우월하다고 말하지 않는다.
...
predictors_nb = [p for p in PREDICTORS if p not in ("logRS", "logRA")]
formula_nb = "W ~ logRS + logRA + " + " + ".join(predictors_nb)
...
if wlist:
    print("Warnings during NB fit:")
    for msg in wlist:
        print(msg.message)
else:
    print("No warnings during NB fit.")

print(full_nb.summary())
```
:::

- 음이항 회귀 적합 결과, R에서는 `iteration 제한에 도달했습니다`(또는 유사한 수렴 경고)가 출력되고, Python에서도 `ConvergenceWarning`·`maxiter` 관련 경고가 함께 나타난다.
- R `warnings()[1]`과 Python 경고 메시지가 같은 유형(수렴 실패)이면, 두 언어 모두 음이항 모형이 이 데이터에 대해 정상적으로 적합되지 못한 것으로 같은 결론을 내린다. (양쪽 모두 경고가 없으면 `summary()`의 계수·AIC를 비교한다.)
- 수렴 경고가 발생했으므로 계수 추정치를 최종 결과로 바로 해석하기보다, 원인(과소·과대산포)을 확인하기 위해 동일 설명변수를 사용한 포아송 모형의 산포 비율을 계산한다.

- **해석 기준 (음이항 적합)**: 위 세 항이 모두 해당하면 “음이항 적합 실패 → 산포 비율 진단으로 넘어간다”는 흐름으로 서술한다.
- 음이항 모형이 수렴하지 못한 이유를 파악하기 위해, 동일한 변수를 넣은 포아송 회귀모형의 산포 비율(Dispersion Ratio = 잔차 이탈도 / 잔차 자유도)을 계산한다.
...
```{r}
# 산포 비율 계산 (full_poisson은 문제 2-3에서 적합)
...
```{python}
dispersion_ratio = full_poisson.deviance / full_poisson.df_resid
print(f"Dispersion Ratio : {dispersion_ratio}")
```
:::

- **해석 기준 (산포 비율)**: R과 Python의 Dispersion Ratio가 모두 1보다 훨씬 작으면(예: 0.1~0.2대) 과소산포로 같은 해석을 한다. 두 값이 소수점 셋째 자리까지 비슷하면 계산도 일치한 것이다.
- **해석 기준 (산포·음이항 종합)**: R/Python 모두 수렴 경고가 뜨고 Dispersion Ratio가 0.13처럼 1보다 훨씬 작으면, 과소산포로 인해 `glm.nb()`가 실패한 것으로 같은 결론을 내린다.
...
- 제시된 Bill James 식에 로짓 변환을 취하면
  $\text{logit}(WPct_{i,t}) = (k + g\,I_t)\,\log(RS_{i,t}/RA_{i,t})$
  가 된다. 여기서 $I_t = I(1994 \leq t \leq 2005)$이다.
- $g=0$이면 두 시대의 지수 $k$가 같고, $g \neq 0$이면 스테로이드 시대(1994–2005)의 지수가 $k+g$로 달라진다.
- 따라서 절편 없는 로지스틱 모형
  $\text{logit}(WPct) = k\,\log(RS/RA) + g\,I\,\log(RS/RA)$
  를 적합하고, `steroid:logRR` 계수(또는 중첩모형 LRT)로 $H_0: g=0$을 검정한다.
- 데이터는 1994–2005년과 2010–2025년을 포함하고 2020년은 제외한다. 가중치는 `W + L`을 사용한다.
...
# g = 0 검정 (중첩모형 LRT)
g_test <- anova(model_k, model_kg, test = "Chisq")

coef_kg <- summary(model_kg)$coefficients
g_coef_name <- setdiff(names(coef(model_kg)), names(coef(model_k)))
p_col <- grep("^Pr\\(", colnames(coef_kg), value = TRUE)[1]
g_wald_p <- if (length(g_coef_name) == 1L) coef_kg[g_coef_name, p_col] else NA_real_

output_24 <- paste0(
  "Nested model test for g = 0\n",
  paste(capture.output(print(g_test)), collapse = "\n"), "\n\n",
  "Coefficients (model with g)\n",
  paste(capture.output(print(round(coef_kg, 4))), collapse = "\n"), "\n",
  if (length(g_coef_name) == 1L) {
    paste0("Wald p-value for ", g_coef_name, " (g): ", format.pval(g_wald_p, digits = 4), "\n")
  } else {
    "Wald p-value for g: could not identify interaction coefficient\n"
  }
)
cat(output_24)
```
...
Teams_24 = pl.from_pandas(pylahman.Teams())
...
teams_24 = (
    ...
    .with_columns([
        (pl.col("W") / (pl.col("W") + pl.col("L"))).alias("WPct"),
        (pl.col("R") / pl.col("RA")).log().alias("logRR"),
        (
            (pl.col("yearID") >= 1994) & (pl.col("yearID") <= 2005)
        ).cast(pl.Int8).alias("steroid"),
    ])
    .select(["WPct", "W", "L", "logRR", "steroid"])
    .to_pandas()
)

teams_24["steroid_logRR"] = teams_24["steroid"] * teams_24["logRR"]
trial_w = teams_24["W"] + teams_24["L"]

model_k = smf.glm(
    formula="WPct ~ 0 + logRR",
    data=teams_24,
    family=sm.families.Binomial(),
    var_weights=trial_w,
).fit()
...
model_kg = smf.glm(
    formula="WPct ~ 0 + logRR + steroid_logRR",
    data=teams_24,
    family=sm.families.Binomial(),
    var_weights=trial_w,
).fit()

lr_stat = 2 * (model_kg.llf - model_k.llf)
lr_p = chi2.sf(lr_stat, df=1)
g_wald_p = model_kg.pvalues["steroid_logRR"]

coef_kg = pd.DataFrame({
    "Estimate": model_kg.params,
    "Std. Error": model_kg.bse,
    "z value": model_kg.tvalues,
    "Pr(>|z|)": model_kg.pvalues,
}).round(4)
...
output_24 = (
    "Nested model test for g = 0\n"
    f"{g_test.round(4).to_string(index=False)}\n\n"
    "Coefficients (model with g)\n"
    f"{coef_kg.to_string()}\n"
    f"Wald p-value for steroid_logRR (g): {g_wald_p:.4g}\n"
)
print(output_24)
```
:::

- **해석 기준 ($k$ 추정)**: `logRR` 계수는 최근 시대(2010–2025)의 $k$에 해당한다. R과 Python에서 이 값이 소수점 둘째~셋째 자리까지 같으면(예: 1.7~1.8대) `문제 2-1`의 $k$ 추정과 같은 결론을 내린다.
- **해석 기준 ($g$ 검정)**: 중첩모형 LRT p-value 또는 `steroid:logRR` Wald p-value가
  - **0.05 미만**이면 $g \neq 0$으로, 스테로이드 시대와 최근 시대의 $k$가 **유의하게 다르다**고 해석한다.
  - **0.05 이상**이면 $g=0$을 기각하지 못하므로, 두 시대의 $k$ 차이를 **통계적으로 확인할 수 없다**고 해석한다.
- **해석 기준 (R vs Python)**: $k$, $g$ 점추정치·p-value·LRT p-value가 양쪽에서 같으면 동일한 가설검정 결론이다. `문제 2-1` 2항과 마찬가지로, 모형 표기(`steroid:logRR` vs `steroid_logRR`)가 달라도 계수가 같으면 같은 추정으로 본다.
- LRT p-value와 `steroid:logRR`의 Wald p-value가 모두 0.05 이상이면, 스테로이드 시대(1994–2005)와 최근 시대(2010–2025) 사이에 Bill James 지수 $k$의 유의한 차이($g \neq 0$)는 확인되지 않는다.
- `logRR` 계수(최근 시대의 $k$)는 `문제 2-1`에서 추정한 $k$와 같은 범위(약 1.7~1.8)로 나타나며, 두 시대를 합친 분석에서도 득실점 비율이 승률을 설명하는 핵심 변수임을 다시 확인할 수 있다.
- `steroid:logRR` 계수 $g$의 부호가 양(+)이면 스테로이드 시대의 지수가 더 크고, 음(–)이면 더 작다. 다만 p-value가 0.05 이상이면 그 차이는 통계적으로 유의하지 않다.
